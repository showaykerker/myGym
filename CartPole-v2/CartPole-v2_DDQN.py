...
class DQNCartPoleSolver():
	def __init__(self, n_episodes=10000, gamma=1.0, epsilon=1.0, epsilon_min=0.0,
	...
	def run(self):
		...
		for e in range(self.n_episodes):
			...
			while not done:
				...
				if step_ == 0 or step_ > 200: self.env.render() 
				...
				score += reward
				step_ += 1
				if step_ % 1000 == 0 and step_ != 0 and not done: print('... Current Time Step', step_)

			print('Ep No. %d, step %d, score %.2f' %(e+1, step_, score))
